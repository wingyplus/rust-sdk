...
fn enum_impl(declared: &parse::Enum) -> String {
    ...
    for variant in &declared.variants {
        members.push_str(&format!(
            "::dagger::__private::EnumMemberDef {{ name: {name}, doc: {doc} }},",
    ...
    format!(
        r#"
impl ::dagger::__private::EnumType for {ty} {{
    const DEF: ::dagger::__private::EnumDef = ::dagger::__private::EnumDef {{
        name: {name},
        doc: {doc},
        members: &[{members}],
    }};
...
fn state_impl(def: &parse::StructDef, enums: &Enums) -> Result<String, String> {
    ...
    for field in &def.fields {
        ...
        defs.push_str(&format!(
            "::dagger::__private::FieldDef {{ name: {name}, kind: {kind}, type_name: {type_of}, list: {list}, optional: {optional}, doc: {doc}, deprecated: {deprecated}, source: {source} }},",
    ...
    Ok(format!(
        r#"
impl ::dagger::__private::ObjectState for {type_name} {{
    fn fields() -> &'static [::dagger::__private::FieldDef] {{
        const FIELDS: &[::dagger::__private::FieldDef] = &[{defs}];
        FIELDS
    }}

    #[allow(unused_variables)]
    fn from_state(
        state: &::dagger::__private::State,
    ...
    fn to_state(&self) -> ::core::result::Result<::goish::gostring::string, ::goish::gostring::string> {{
        let mut __state = ::dagger::__private::StateWriter::new();
        {writes}
...
fn object_impl(block: &parse::ImplBlock, enums: &Enums) -> Result<String, String> {
    ...
    Ok(format!(
        r#"
impl ::dagger::__private::Object for {type_name} {{
    const NAME: &'static str = {name_literal};

    const DOC: &'static str = {doc_literal};

    const SOURCE: ::dagger::__private::SourceMapDef = {source};

    fn functions() -> &'static [::dagger::__private::FunctionDef] {{
        const FUNCTIONS: &[::dagger::__private::FunctionDef] = &[{defs}];
        FUNCTIONS
    }}
{enum_defs}{constructor}
    ...
    fn invoke(
        self,
        name: &::goish::gostring::string,
        args: &::dagger::__private::Arguments,
...
fn constructor_impl(type_name: &str, f: &Function, enums: &Enums) -> Result<String, String> {
    ...
    Ok(format!(
        r#"
    fn constructor() -> ::core::option::Option<&'static ::dagger::__private::FunctionDef> {{
        const CONSTRUCTOR: ::dagger::__private::FunctionDef = {def}
    ...
    fn construct(
        ...
        {bindings}
        ::dagger::__private::ObjectState::to_state(&{call})
    }}
"#,
...
fn source_map_def(loc: &SourceLoc) -> String {
    if loc.file.is_empty() {
        return "::dagger::__private::SourceMapDef::UNKNOWN".to_string();
    }
    format!(
        "::dagger::__private::SourceMapDef {{ file: {file}, line: {line}, column: {column} }}",
...
fn enum_defs(enums: &Enums) -> String {
    ...
    let listed = enums
        .paths
        .iter()
        .map(|path| format!("<{path} as ::dagger::__private::EnumType>::DEF,"))
        .collect::<String>();
    format!(
        r#"
    fn enums() -> &'static [::dagger::__private::EnumDef] {{
        const ENUMS: &[::dagger::__private::EnumDef] = &[{listed}];
        ENUMS
    }}
"#
    )
...
fn function_def_named(
    ...
    for param in &f.params {
        ...
        args.push_str(&format!(
            "::dagger::__private::ArgDef {{ name: {name}, kind: {kind}, type_name: {type_name}, list: {list}, optional: {optional}, doc: {doc}, default_value: {default}, default_path: {path}, ignore: &[{ignore}], deprecated: {deprecated}, source: {source} }},",
    ...
    Ok(format!(
        "::dagger::__private::FunctionDef {{ name: {name}, doc: {doc}, return_kind: {ret}, return_type_name: {ret_type_name}, return_list: {ret_list}, return_optional: {ret_optional}, is_check: {is_check}, generator: {generator}, deprecated: {deprecated}, source: {source}, args: &[{args}] }},",
        name = quote_str(api_name),
        doc = quote_str(&f.doc),
    ...
    match kind.kind {
        "OBJECT_KIND" => {
            ...
            let rebuild = match unwrap_list(ty) {
                Some(element) => format!("::dagger::__private::from_ids::<{element}>"),
        ...
        "ENUM_KIND" => {
            let from_member = format!("<{ty} as ::dagger::__private::EnumType>::from_member");
            if kind.optional {
                format!(
...
fn write_value(name: &str, kind: &Kind) -> Result<String, String> {
    ...
    let (encoder, borrow, deref, question) = match kind.kind {
        "STRING_KIND" => ("::dagger::__private::encode_string", "&", "", ""),
        "INTEGER_KIND" => ("::dagger::__private::encode_int", "", "*", ""),
        "FLOAT_KIND" => ("::dagger::__private::encode_float", "", "*", ""),
        "BOOLEAN_KIND" => ("::dagger::__private::encode_bool", "", "*", ""),
        // Fallible, unlike the others: reading a generated object's id runs the
        // chain it was built from.
        "OBJECT_KIND" => ("::dagger::__private::encode_object", "&", "", "?"),
        // An enum goes back as the member's name, which the value already
        // carries — nothing to fetch, so nothing to fail.
        "ENUM_KIND" => ("::dagger::__private::encode_enum", "&", "", ""),
...
fn optional_write(name: &str, encode: &str) -> String {
    format!(
        "match &self.{name} {{ ::core::option::Option::Some(value) => {encode}, ::core::option::Option::None => ::dagger::__private::encode_null() }}"
    )
}
...
fn wrap_failure(call: String, failure: &Failure) -> String {
    match failure {
        ...
        Failure::GoError => format!("({call}).map_err(::dagger::__private::error_message)?"),
    }
}
...
fn list_encoder(kind: &str, call: &str) -> Result<String, String> {
    Ok(match kind {
        "STRING_KIND" => format!("::dagger::__private::encode_string_list(&{call})"),
        "INTEGER_KIND" => format!("::dagger::__private::encode_int_list(&{call})"),
        "FLOAT_KIND" => format!("::dagger::__private::encode_float_list(&{call})"),
        "BOOLEAN_KIND" => format!("::dagger::__private::encode_bool_list(&{call})"),
        "OBJECT_KIND" => format!("::dagger::__private::encode_object_list(&{call})?"),
...
fn dispatch_arm(type_name: &str, f: &Function, enums: &Enums) -> Result<String, String> {
    ...
    let encode = if ret.optional {
        let some = if ret.list {
            ...
            match ret.kind {
                "STRING_KIND" => "::dagger::__private::encode_string(&__value)".to_string(),
                "INTEGER_KIND" => "::dagger::__private::encode_int(__value)".to_string(),
                "FLOAT_KIND" => "::dagger::__private::encode_float(__value)".to_string(),
                "BOOLEAN_KIND" => "::dagger::__private::encode_bool(__value)".to_string(),
                ...
                "OBJECT_KIND" if ret.is_object(type_name) => {
                    "::dagger::__private::encode_state(&__value)?".to_string()
                }
                "OBJECT_KIND" => "::dagger::__private::encode_object(&__value)?".to_string(),
                // The member's name is already in the value, so nothing is
                // fetched and nothing can fail — the `Some` arm needs no `?`.
                "ENUM_KIND" => "::dagger::__private::encode_enum(&__value)".to_string(),
                other => {
        ...
        format!(
            "match {call} {{ ::core::option::Option::Some(__value) => {some}, ::core::option::Option::None => ::dagger::__private::encode_null() }}"
        )
    } else {
        match ret.kind {
            ...
            "VOID_KIND" => format!("{{ {call}; ::dagger::__private::encode_void() }}"),
            "STRING_KIND" => format!("::dagger::__private::encode_string(&{call})"),
            "INTEGER_KIND" => format!("::dagger::__private::encode_int({call})"),
            "FLOAT_KIND" => format!("::dagger::__private::encode_float({call})"),
            "BOOLEAN_KIND" => format!("::dagger::__private::encode_bool({call})"),
            ...
            "OBJECT_KIND" if ret.is_object(type_name) => {
                format!("::dagger::__private::encode_state(&{call})?")
            }
            // Fallible, unlike the others: reading a generated object's id runs
            // the chain the function built.
            "OBJECT_KIND" => format!("::dagger::__private::encode_object(&{call})?"),
            // An enum goes back as the member's name, which the value already
            // carries — nothing to fetch, so nothing to fail.
            "ENUM_KIND" => format!("::dagger::__private::encode_enum(&{call})"),
