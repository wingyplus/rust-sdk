...
fn a_returned_object_is_encoded_as_its_id() {
    ...
    assert!(
        arm.contains("::dagger::__private::encode_object(&self.base())?"),
...
fn a_list_argument_and_return_are_declared_as_lists() {
    ...
    assert!(
        arm.contains("::dagger::__private::encode_string_list(&self.tags(names))"),
...
fn a_list_of_floats_uses_the_float_accessor_and_encoder() {
    ...
    assert!(
        arm.contains("::dagger::__private::encode_float_list(&self.halved(numbers))"),
...
fn a_list_of_objects_goes_through_its_ids() {
    ...
    assert!(
        arm.contains(r#"let dirs = ::dagger::__private::from_ids::<gen::Directory>(args.object_list("dirs")?);"#),
        "rebuilt from its ids: {arm}"
    );
    // Fallible for the reason a single object's is: each element's id is a
    // round trip.
    assert!(
        arm.contains("::dagger::__private::encode_object_list(&self.mount(dirs))?"),
...
fn an_optional_list_is_read_only_when_present() {
    ...
    assert!(
        arm.contains(r#"args.object_list_opt("dirs")?.map(::dagger::__private::from_ids::<Directory>)"#),
        "rebuilt only when present: {arm}"
    );
}
...
fn an_optional_return_encodes_none_as_null() {
    for (ty, some) in [
        ("Option<string>", "::dagger::__private::encode_string(&__value)"),
        ("Option<int>", "::dagger::__private::encode_int(__value)"),
        ("Option<f64>", "::dagger::__private::encode_float(__value)"),
        ("Option<bool>", "::dagger::__private::encode_bool(__value)"),
        ("Option<Container>", "::dagger::__private::encode_object(&__value)?"),
        ...
        ("Option<slice<string>>", "::dagger::__private::encode_string_list(&__value)"),
        ("Option<slice<int>>", "::dagger::__private::encode_int_list(&__value)"),
        ("Option<Vec<Directory>>", "::dagger::__private::encode_object_list(&__value)?"),
        // An enum goes back as its member's name, which the value already
        // carries — so unlike the object arm this one has no `?`.
        ("Option<TargetOs>", "::dagger::__private::encode_enum(&__value)"),
    ] {
        let f = function("maybe", Vec::new(), ty);
        ...
        assert!(
            arm.contains(&format!(
                "match self.maybe() {{ ::core::option::Option::Some(__value) => {some}, ::core::option::Option::None => ::dagger::__private::encode_null() }}"
            )),
...
fn a_fallible_optional_return_composes_both() {
    ...
    assert!(
        arm.contains("match (self.maybe())? { ::core::option::Option::Some(__value) => ::dagger::__private::encode_string(&__value), ::core::option::Option::None => ::dagger::__private::encode_null() }"),
...
fn a_fallible_return_is_passed_through_with_a_question_mark() {
    for (ty, encoded) in [
        ("Result<string, string>", "::dagger::__private::encode_string(&(self.attempt())?)"),
        ("Result<int, string>", "::dagger::__private::encode_int((self.attempt())?)"),
        ("Result<f64, string>", "::dagger::__private::encode_float((self.attempt())?)"),
        ("Result<bool, string>", "::dagger::__private::encode_bool((self.attempt())?)"),
        ("Result<(), string>", "{ (self.attempt())?; ::dagger::__private::encode_void() }"),
        // Two `?`s, and they are different failures: the inner one is the
        // function's own, the outer is resolving the object it returned.
        ("Result<Container, string>", "::dagger::__private::encode_object(&(self.attempt())?)?"),
        // A goish `error` is a value, not a message, so it is read through the
        // helper — `Error()` inline would panic on the nil one.
        (
            "Result<string, error>",
            "::dagger::__private::encode_string(&(self.attempt()).map_err(::dagger::__private::error_message)?)",
        ),
        (
            "Result<(), goish::error>",
            "{ (self.attempt()).map_err(::dagger::__private::error_message)?; ::dagger::__private::encode_void() }",
        ),
    ] {
        let f = function("attempt", Vec::new(), ty);
...
fn a_source_location_becomes_a_source_map() {
    ...
    assert_eq!(
        ...
        r#"::dagger::__private::SourceMapDef { file: "src/main.rs", line: 42, column: 9 }"#
    );
    assert_eq!(
        source_map_def(&SourceLoc::unknown()),
        "::dagger::__private::SourceMapDef::UNKNOWN"
    );
}
...
fn a_function_and_its_arguments_carry_their_source() {
    ...
    assert!(
        def.contains(r#"source: ::dagger::__private::SourceMapDef { file: "src/main.rs", line: 12, column: 5 }"#),
        "the function's own location: {def}"
    );
    assert!(
        def.contains(r#"source: ::dagger::__private::SourceMapDef { file: "src/main.rs", line: 13, column: 9 }"#),
...
fn the_impl_doc_becomes_the_object_doc() {
    ...
    assert!(
        generated.contains(
            r#"const SOURCE: ::dagger::__private::SourceMapDef = ::dagger::__private::SourceMapDef { file: "src/main.rs", line: 7, column: 6 };"#
        ),
...
fn an_undocumented_impl_declares_an_empty_doc() {
    ...
    assert!(
        generated.contains("const SOURCE: ::dagger::__private::SourceMapDef = ::dagger::__private::SourceMapDef::UNKNOWN;"),
...
fn a_float_argument_and_return_use_the_float_accessor_and_encoder() {
    ...
    assert!(
        arm.contains("::dagger::__private::encode_float(self.scale(factor))"),
...
fn enum_arguments_and_returns_go_through_from_member() {
    ...
    assert!(
        arm.contains(
            r#"let os = <TargetOs as ::dagger::__private::EnumType>::from_member(&args.enum_member("os")?)?;"#
        ),
        "read as a member name: {arm}"
    );
    assert!(
        arm.contains("::dagger::__private::encode_enum(&self.build(os))"),
...
fn an_optional_enum_argument_keeps_its_failure_outside_the_option() {
    ...
    assert!(
        arm.contains(r#"match args.enum_member_opt("os")?"#)
            && arm.contains("<TargetOs as ::dagger::__private::EnumType>::from_member(&member)?"),
...
fn declared_enums_reach_the_object_impl() {
    ...
    assert!(
        listed.contains("<TargetOs as ::dagger::__private::EnumType>::DEF,")
            && listed.contains("<gen::Level as ::dagger::__private::EnumType>::DEF,"),
...
fn an_enum_declares_its_variants_and_their_docs() {
    ...
    assert!(
        emitted
            .contains(r#"::dagger::__private::EnumMemberDef { name: "Alpine", doc: "Alpine Linux." },"#),
    ...
    assert!(
        emitted.contains(r#"::dagger::__private::EnumMemberDef { name: "DebianTesting", doc: "" },"#),
...
fn a_field_is_read_and_written_by_its_kind() {
    ...
    for want in [
        ...
        r#"mounts: ::dagger::__private::from_ids::<gen::Directory>(state.object_list("mounts")?),"#,
        r#"tags: state.string_list("tags")?,"#,
        r#"__state.put("image", ::dagger::__private::encode_string(&self.image));"#,
        r#"__state.put("jobs", ::dagger::__private::encode_int(self.jobs));"#,
        r#"__state.put("source", ::dagger::__private::encode_object(&self.source)?);"#,
        r#"__state.put("mounts", ::dagger::__private::encode_object_list(&self.mounts)?);"#,
        r#"__state.put("tags", ::dagger::__private::encode_string_list(&self.tags));"#,
    ] {
        assert!(generated.contains(want), "emits {want}: {generated}");
    }
    assert!(
        generated.contains("::dagger::__private::encode_null()"),
...
fn a_unit_struct_declares_no_fields() {
    ...
    assert!(
        generated.contains("const FIELDS: &[::dagger::__private::FieldDef] = &[];"),
...
fn a_constructor_is_registered_with_no_name() {
    ...
    assert!(
        generated
            .contains(r#"const CONSTRUCTOR: ::dagger::__private::FunctionDef = ::dagger::__private::FunctionDef { name: "","#),
    ...
    assert!(
        generated.contains("const FUNCTIONS: &[::dagger::__private::FunctionDef] = &[];"),
    ...
    assert!(
        generated.contains("::dagger::__private::ObjectState::to_state(&Build::new(image))"),
...
fn a_constructor_may_be_written_fallibly() {
    ...
    assert!(
        generated.contains("::dagger::__private::ObjectState::to_state(&(Build::new())?)"),
...
fn returning_the_objects_own_type_encodes_its_state() {
    ...
    assert!(
        arm.contains("::dagger::__private::encode_state(&self.with_tag(tag))?"),
