...
| --- | --- | --- |
| `declaration` | `default`, `doc`, `deprecated` on an argument and on a function, the `impl` block's own doc comment, source maps, `Option<T>` arguments and returns, lists in and out (including an empty one and an optional one), Void returns, fallible returns in both error types, enums the module declares, failure propagation and the message that comes with it | `sdk/macros` is a proc-macro crate; the `proc_macro` API panics outside a macro expansion, so its own tests build `Function` values by hand and never parse a signature |
| `objects` | `Directory` in, a list of them in, `Container` out (and an optional one), `default_path`, `ignore` | an object crosses the boundary as an engine ID, and nothing until now called `ObjectId::from_id`/`to_id` against a real engine |
| `state` | `pub` fields as `withField`, a `#[dagger::constructor]` and its defaults, a function returning `Self`, state surviving a chain of calls, a field's description and deprecation | `sdk/tests/state.rs` compiles and runs what the macros emit, but sends no query: only an engine answers a field without running the module, and only an engine hands one call's document back as the next one's parent |
